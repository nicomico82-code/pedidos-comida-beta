...
const whatsappUrl = (phone: string, text: string) => `https://wa.me/${phone.replace(/\D/g, "")}?text=${encodeURIComponent(text)}`;
...
export default function App() {
  ...
  if (order) {
    const detail = order.items.map((item) => `${item.quantity}x ${item.name}`).join(", ");
    const text = `Hola, recibimos un nuevo pedido en ${catalog.business.businessName}. Pedido ${order.orderCode}: ${detail}. Total: ${money(order.totalClp)}. Cliente: ${order.customerName}. Modalidad: ${order.fulfillment === "retiro" ? "Retiro en local" : "Despacho"}.`;
    const notify = () => { if (!catalog.business.whatsapp) { window.alert("El negocio aún no tiene un WhatsApp configurado."); return; } window.open(whatsappUrl(catalog.business.whatsapp, text), "_blank", "noopener,noreferrer"); };
    return <main className="success"><div className="check">✓</div><p className="eyebrow">PEDIDO RECIBIDO</p><h1>¡Gracias, {order.customerName}!</h1><p>Tu pedido quedó registrado con el código <b>{order.orderCode}</b>.</p><div className="order-card"><span>Estado</span><b>Recibido</b><span>Total</span><b>{money(order.totalClp)}</b><span>Modalidad</span><b>{order.fulfillment === "retiro" ? "Retiro en local" : "Despacho"}</b></div><div style={{ display: "grid", gap: 14, width: "100%", maxWidth: 640, margin: "0 auto" }}><button className="primary whatsapp" onClick={notify}>Avisar pedido por WhatsApp</button><button onClick={() => setOrder(undefined)}>Hacer otro pedido</button></div></main>;
  }
